  ...
  return (
    <CardBody>
      ...
      <Heading as="h4" color="secondary" mb="16px">
        {t('StakeCNR in IFO pool')}
      </Heading>
      <Box>
        <Text color="textSubtle" small>
          {t(
            'The maximum amount ofCNR user can commit to the Public Sale, is equal to the averageCNR balance in the IFOCNR pool prior to the IFO. Stake moreCNR to increase the maximumCNR you can commit to the sale. Missed this IFO? You can keep staking in the IFOCNR Pool to join the next IFO sale.',
          )}
        </Text>
      ...
      {hasProfile && (
        <SmallStakePoolCard borderRadius="default" p="16px">
          <FlexGap justifyContent="space-between" alignItems="center" flexWrap="wrap" gap="16px">
            ...
            {isVaultApproved ? (
              <Button onClick={onPresentStake}>{t('Stake')}CNR</Button>
            ) : (
...
const Step2 = ({ hasProfile, isLive, isCommitted }: { hasProfile: boolean; isLive: boolean; isCommitted: boolean }) => {
  ...
  return (
    <CardBody>
      ...
      <Text color="textSubtle" small>
        {t('When the IFO sales are live, you can “commit” yourCNR to buy the tokens being sold.')} <br />
      </Text>
      ...
      case 3:
        return (
          <CardBody>
            ...
            <Text color="textSubtle" small>
              {t(
                'After the IFO sales finish, you can claim any IFO tokens that you bought, and any unspentCNR tokens will be returned to your wallet.',
              )}
            </Text>
          </CardBody>
        )
