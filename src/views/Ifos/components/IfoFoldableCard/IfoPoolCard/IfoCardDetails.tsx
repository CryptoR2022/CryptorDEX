...
const MaxTokenEntry = ({ maxToken, ifo, poolId }: { maxToken: number; ifo: Ifo; poolId: PoolIds }) => {
  const isCurrencyCake = ifo.currency === tokens.cnr
  ...
  const basicTooltipContent =
    ifo.version === 3.1
      ? t(
          'For the private sale, each eligible participant will be able to commit any amount of CNR up to the maximum commit limit, which is published along with the IFO voting proposal.',
        )
      : t(
          'For the basic sale, Max CNR entry is capped by minimum between your average CNR balance in the IFO CNR pool, or the pool’s hard cap. To increase the max entry, Stake more CNR into the IFO CNR pool',
        )

  const unlimitedToolipContent =
    ifo.version === 3.1
      ? t(
          'For the public sale, Max CNR entry is capped by your average CNR balance in the IFO CNR pool. To increase the max entry, Stake more CNR into the IFO CNR pool',
        )
      : t(
          'For the unlimited sale, Max CNR entry is capped by your average CNR balance in the IFO CNR pool. To increase the max entry, Stake more CNR into the IFO CNR pool',
        )

  const tooltipContent = poolId === PoolIds.poolBasic ? basicTooltipContent : unlimitedToolipContent

  const { targetRef, tooltip, tooltipVisible } = useTooltip(tooltipContent, { placement: 'bottom-start' })
  const label = isCurrencyCake ? t('Max. CNR entry') : t('Max. token entry')
    ...
    if (status === 'coming_soon') {
      return (
        <>
          ...
          {ifo[poolId].cakeToBurn !== '$0' && <FooterEntry label={t('CNR to burn:')} value={ifo[poolId].cakeToBurn} />}
          <FooterEntry
